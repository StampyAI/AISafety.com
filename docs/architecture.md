...
- **Activity trail** (`audit.ts`): sign-ins, access requests, approvals and additions, tab changes, removals, dismissed requests and newsletter sends are appended to a capped Redis list (`admin:audit`, 2000 events; `.admin-dev/audit.json` locally) and shown newest-first under "Recent activity" on Admin admin. Logging is best effort and never fails the action.
