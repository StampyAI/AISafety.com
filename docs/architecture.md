...
```
 ...
 │ src/app/admin/*           queue · analytics · chat log ·           │
 │                           map editor · preview · newsletter        │
...
- **Map editor.** `/admin/map` (`mapEditor` area; `'edit'` to move anything, `'view'` shows the same map and side panel with nothing to drag, place or resize, `applyChange` being the one place every change passes through) lets the owner drag logos on the Field map and change a logo's size. It writes to Airtable through `/api/admin/map` (GET live records, PATCH one change). It is deliberately a **copy** of the public map's rendering, not a shared component: `map-geometry.ts` mirrors the constants in `src/app/map/D3Map.tsx` and `map-geometry.test.ts` fails if they drift. The editor reads Airtable directly with `cache: 'no-store'` (unpublished rows included, Hide? rows excluded), never through `fetchAirtableRecords`, and never revalidates anything, so `/map`'s data path, cache and bundle are untouched. The only fields it can write are `x` and `y` together, or `Scale` as one of its existing options (`map-editor-core.ts`; any other key in the request body is rejected). Publish? and Hide? stay in Airtable.

- **Queue.** `/admin/queue` (`queue` area; `'edit'` to decide anything, `'view'` shows the same inbox with a "View only" line in place of Accept / Reject / Undo and every field read-only, `act()` a no-op) is one review inbox for everything waiting on the owner: a person's email or Discord request, a Broom flag, a rulebook edit written from earlier reject reasons, a Comb suggestion. Rows live in the Airtable table **Queue** (`tblonlKwIFJ7Aa8QN`, `src/lib/admin/queue.ts` holds the field ids) and are written by the bots on the owner's Mac (`~/Queue`); the site only lists them and carries out decisions through `/api/admin/queue` (GET list, POST `{id, action}` with `accept` / `reject` / `revise` / `undo`). The row carries the proposal as JSON: for an **Add** the proposed record's fields (accept ticks `Publish?` on the target record, plus any values edited on the page); for a **Change** a list of `{field, from, to}` (accept writes the `to` values by field NAME and deletes the Broom flag row; reject deletes the flag row too); for a **Rule** a diff (accept only marks the row Accepted, the Mac worker patches the file). Every accept re-reads the row and refuses one already decided (409), then calls `revalidateTag('airtable-records', 'max')` so the public pages refresh within a few minutes. Reject needs a reason (the page offers chips Fable wrote at intake); a rejected suggestion is deleted by the Mac worker 24 hours later, which is the undo window. `Publish?` and `Hide?` can never be set through edits or proposals (`sanitiseEdits`, `PROTECTED_FIELDS`). Nothing auto-applies, ever. An emailed request (Source Email, written by the owner's inbox bot) also carries a reply draft in the owner's voice: the page shows it editable, an accept writes the approved text to the row, and then the page calls a small **agent on the owner's Mac** (loopback HTTP, port from `QUEUE_AGENT_PORT`, default 8790) to save it as a Gmail draft at once, never to send it. `GET /api/admin/queue` hands the page `{ port, token }` for that call, the token sealed with `QUEUE_AGENT_SECRET` (both sides hold it; unset = no agent, the Mac worker saves the draft on its next pass). The agent only acts on rows Airtable says are already accepted, so the token can do nothing the queue has not approved. The page opens the item named in a `#rec…` hash, which is how the bot's "Queued:" links land on the right card, and it keeps the hash in step with the item in focus (replaced, never pushed) so the address bar is always a link to that one suggestion.

- **Newsletter approval.** `/admin/newsletter` (`newsletter` area; `'edit'` to approve, `'view'` opens the same page read-only — drafts and previews, no Approve button, send API refuses — for a design reviewer or a second pair of eyes) is the approval step for the weekly newsletters. The pipeline on the owner's machine renders a draft into a branded email and creates a **draft** ActiveCampaign campaign whose message carries a hidden content marker (`<!--aisafety-issue:<checksum>-->`). The page lists those drafts via `/api/admin/newsletter` (GET), re-running the pipeline's own checks on each: still a draft, wired to exactly one list (per-list one-click unsubscribe depends on it), one message, marker present and matching a fresh checksum of the HTML (`contentDigest` in `newsletter.ts`; `newsletter.test.ts` pins it to the pipeline's fixtures). A missing marker means someone saved the email in ActiveCampaign's visual designer, which wipes injected HTML, and the page refuses to send. The preview is the stored HTML in a sandboxed iframe (`/api/admin/newsletter/preview?draft=ID`; `draft=` because ad blockers refuse `campaign=` URLs, and CSP `frame-ancestors` rather than X-Frame-Options, which would reject the sandbox's opaque origin). Approving (POST) re-verifies, then schedules the send through ActiveCampaign's legacy v1 API, the only one that can schedule, by creating the sending campaign from the verified message two minutes out and deleting the draft shell. Env: `ACTIVECAMPAIGN_URL`, `ACTIVECAMPAIGN_KEY` (production only).
