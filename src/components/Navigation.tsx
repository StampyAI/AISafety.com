...
// Whole-page previews only make sense for a pointer on a desktop screen,
// outside any preview frame (a previewed page must never preview the nav in
// turn), and not when the browser asks to save data.
function canPreviewPages(): boolean {
...
// Whole-page hover preview: the real page, live, in a frame that fills the
// window behind the nav (see .nav-tooltip in the CSS). The frame mounts once
// `mounted` is true — the pill was hovered, or the background preload reached
// it — and stays, so the next hover is instant. Decoration for pointer users:
// aria-hidden, and Enter on the pill opens the page itself.
function NavPagePreview({ href, mounted }: { href: string; mounted: boolean }) {
  return (
    <span className={styles['nav-tooltip']} aria-hidden="true">
      {mounted && (
        <iframe
          src={href}
          title=""
          tabIndex={-1}
          scrolling="no"
          className={styles['nav-tooltip-frame']}
        />
      )}
    </span>
  )
...
export default function Navigation({
  ...
  // Pages whose live preview frame is mounted. Frames load in the background
  // one every 400 ms from two seconds after this page settles, so they are
  // ready before the pointer reaches the nav; hovering a pill mounts its
  // frame at once. Nothing is mounted where canPreviewPages() says no.
  const [livePages, setLivePages] = useState<Set<string>>(() => new Set())
  useEffect(() => {
    if (!canPreviewPages()) return
    const timers = navItems.map((item, i) =>
      window.setTimeout(
        () =>
          setLivePages(prev =>
            prev.has(item.href) ? prev : new Set(prev).add(item.href)
          ),
        2000 + i * 400
      )
    )
  ...
  const onPillEnter = (e: ReactPointerEvent<HTMLElement>) => {
    ...
    if (href && canPreviewPages())
      setLivePages(prev => (prev.has(href) ? prev : new Set(prev).add(href)))
          ...
          <nav ref={navRef} className={styles['nav-menu']}>
            {visibleItems.map((item, i) => (
              ...
              >
                ...
                <NavPagePreview
                  href={item.href}
                  mounted={livePages.has(item.href)}
                />
              </Link>
            ))}
