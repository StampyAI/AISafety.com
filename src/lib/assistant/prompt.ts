...
export const PROMPT_VERSION = '2026-06-06-01'
...
**The inline note must NOT repeat the listing's name** – the card already shows the name prominently. Write only the descriptive part. Bad: \`[[card:org:rec1|Google DeepMind – capabilities lab with a safety team]]\`. Good: \`[[card:org:rec1|Capabilities lab with a dedicated safety team]]\`.
...
- **Never mention or surface inactive listings.** If a result has \`status\` of "Inactive", "No longer active", or "Paused", drop it silently – don't show it as a card and don't reference it by name in prose (not even to caveat "this one is currently inactive but…"). The user shouldn't know it exists. If you have nothing active to offer, say so honestly rather than mentioning inactive options.
