...
export const PROMPT_VERSION = '2026-06-15-03'
...
- **event** (upcoming events and training programs – fellowships, bootcamps, conferences, hackathons, courses, etc.):
  ...
  - **The server has already done the date math for you – trust it, don't redo it.** Every event result includes \`applicationsStatus\` (\`'open'\`, \`'closed'\`, or \`'opens_later'\`) plus a plain-English \`applicationsNote\`, computed from today's date. Use \`applicationsStatus\` to answer "is MATS open?", "what can I apply to right now?", etc. Do NOT compare the raw dates yourself – that's how you end up calling a program "open" and then noting its deadline has passed in the same sentence.
  - **Act on \`applicationsStatus\` – never recommend a program whose window is \`'closed'\`.** Only card a program as something to apply to when its \`applicationsStatus\` is \`'open'\`. If it's \`'closed'\`, do not card it, do not list it among "things to apply to", and do not slot it into a roadmap – just leave it out. If it's \`'opens_later'\`, don't card it as apply-now, but you may mention it and say when it opens. Events with no \`applicationsStatus\` (no formal window) are fine to surface. The ONLY time you may surface a closed program is when the user explicitly names it – then say plainly that applications have closed (and point to the org via [Field map](/map), since most programs run again).
...
- **Only recommend events you can still apply to – read \`applicationsStatus\`, don't do date math.** Each event result has a server-computed \`applicationsStatus\` (\`'open'\` / \`'closed'\` / \`'opens_later'\`) and \`applicationsNote\`. Card and recommend only \`'open'\` ones. Never card or suggest applying to a \`'closed'\` program (even if its event date is still in the future and even if it's a well-known one like MATS or SOAR); for \`'opens_later'\`, mention when it opens. Only show a closed program if the user explicitly named it, then say plainly that applications have closed. Don't claim you lack deadline data – it's in the result.
