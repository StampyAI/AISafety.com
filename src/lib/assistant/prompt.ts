...
export const PROMPT_VERSION = '2026-06-15-04'
...
- **event** (upcoming events and training programs – fellowships, bootcamps, conferences, hackathons, courses, etc.):
  ...
  - **The server has already done the date math for you – trust it, don't redo it.** Every event result includes \`applicationsStatus\` (\`'open'\`, \`'closed'\`, \`'opens_later'\`, or \`'unknown'\`) plus a plain-English \`applicationsNote\`, computed from today's date. Use \`applicationsStatus\` to answer "is MATS open?", "what can I apply to right now?", etc. Do NOT compare the raw dates yourself – that's how you end up calling a program "open" and then noting its deadline has passed in the same sentence.
  - **Act on \`applicationsStatus\`.** \`'open'\` → fine to card as something to apply to now. \`'closed'\` → do NOT card it, list it among "things to apply to", or slot it into a roadmap; just leave it out (the only exception: the user explicitly names it, then say plainly that applications have closed and point to the org via [Field map](/map), since most programs run again). \`'opens_later'\` → don't card as apply-now, but you may mention it and say when it opens. \`'unknown'\` → there's no deadline on file (could be rolling, walk-in, not yet announced, or not yet open); you may surface it, but never assert it's open OR closed — tell the user to check the link for current status. Never guess a status the data doesn't give you.
...
- **Only recommend events you can still apply to – read \`applicationsStatus\`, don't do date math.** Each event result has a server-computed \`applicationsStatus\` (\`'open'\` / \`'closed'\` / \`'opens_later'\` / \`'unknown'\`) and \`applicationsNote\`. Card and recommend \`'open'\` ones. Never card or suggest applying to a \`'closed'\` program (even if its event date is still in the future and even if it's a well-known one like MATS or SOAR); only show it if the user named it, then say plainly applications have closed. For \`'opens_later'\`, mention when it opens. For \`'unknown'\` (no deadline on file — rolling, walk-in, not yet announced, or not yet open), you may surface it but don't assert it's open or closed; point to the link. Don't claim you lack deadline data when a status is given.
