...
• \`filters\` — optional object of meta-field constraints. Values can be a string OR an array of strings (array means OR semantics: matches if ANY value substring matches). Substring match is case-insensitive.
  ...
  Per type:
    ...
    training: type ("Fellowship"|"Course"|"Bootcamp"|"Other"), mode ("Online"|"In person"|"Hybrid"|"Online or in person"), location (free text like events), focus ("General"|"Technical"|"Governance & policy"), entryBar ("Low"|"Mid"|"High"), timeCommitment ("Full-time"|"Part-time"), stipend ("No stipend"|"Expenses covered"|"Stipend included"), recurring ("Yes" — only evergreen programs carry it)
